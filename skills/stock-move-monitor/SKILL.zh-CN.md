...
这个 skill 用来把一份 A 股自选股列表变成每日异动监控简报。默认流程使用东方财富公开行情和 K 线接口；如果本机已安装 `efinance`，A 股 K 线失败时会自动作为兜底源。`akshare` 可通过 `STOCK_MONITOR_ENABLE_AKSHARE=1` 作为深度兜底，但默认不启用。脚本会计算简单异动信号，并输出 Markdown 日报和 CSV 明细表。
...
默认脚本不依赖 `efinance`、`akshare`、`adata` 或其他第三方行情 SDK。如果这些包在本机已安装，可以作为可选增强，但必须保留当前这个无 SDK 备份路径。调整数据源前先看 `references/data_providers.md`。
...
基础版使用脚本内置的东方财富公开行情/K 线接口，不要求安装任何行情 SDK。这样拿到 share package 后，只要本地 Python 可联网，就能先跑起来。

当前表里的 `main_net_yi` 和 `main_net_pct` 主要来自东方财富公开行情接口中的资金流向字段，口径属于平台按成交单推算的主力资金，不是交易所官方披露字段。

当前 K 线兜底顺序是：东方财富直连 -> `efinance`（如已安装，仅默认用于 A 股）-> `akshare`（仅在设置 `STOCK_MONITOR_ENABLE_AKSHARE=1` 时启用）。`adata` 可以作为后续可选增强，用于实时行情交叉验证、指数/概念行情和板块联动分析。但不要把任何一个包写成必装依赖；如果可选包不可用、返回空表或网络失败，脚本应继续生成报告，并在日报里标注数据质量。
...
2. 今日重点异动表
3. 个股异动分析
4. 异动原因核验：来源类型、相关性评分、原因分类和一句初步判断
5. 明日重点关注清单

### K线标注图

默认命令行会额外生成 `kline_annotations_YYYY-MM-DD.html`，在最近一段 K 线上用蓝色虚线标出异动日期，并附上异动类型、涨跌幅、量比和原因核验摘要。网页工具里需要勾选“生成K线标注图（较慢）”才会生成这个文件。
